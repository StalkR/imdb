...
var (
	...
	titleRatingRE        = regexp.MustCompile(`(?:class="rating-rating\s*"|itemprop="ratingValue")>.*?([0-9.]+)`)
	...
	titleDirectorsRE     = regexp.MustCompile(`(?s)<span itemprop="director"[^>]+>(.*?)</div>`)
	titleWritersRE       = regexp.MustCompile(`(?s)<span itemprop="creator"[^>]+itemtype="http://schema.org/Person"[^>]*>(.*?)</div>`)
